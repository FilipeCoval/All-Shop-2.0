...
async function main() {
    ...
    for (const publicDoc of publicSnap.docs) {
        ...
        console.log(`Lote ${pid} (${product.name}): Físico=${totalPhysical}, Pendentes=${pendingInOrders}, Reservado=${reservedInCart}, Final=${availableStock}`);

        if (product.stock !== availableStock) {
            updates++;
            batch.update(publicDoc.ref, { stock: availableStock });
            console.log(`-> Atualizando de ${product.stock} para ${availableStock}`);
        }
    }
