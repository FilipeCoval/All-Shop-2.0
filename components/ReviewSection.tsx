...
import {  storage, db , modularDb } from '../services/firebaseConfig';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (currentUser) {
        try {
            ...
            const points = hasPhotos ? 30 : 15;
            const newHistory: PointHistory = {
                id: `review-${newReview.id}`,
                date: new Date().toISOString(),
                amount: points,
                reason: hasPhotos ? 'Avaliação de Produto (com foto)' : 'Avaliação de Produto'
            };
