            ...
            <div className="flex-1 overflow-x-auto p-4 md:p-8">
                <div className="min-w-[800px] mx-auto max-w-7xl">
                    <div className="grid" style={{ gridTemplateColumns: `200px repeat(${products.length}, minmax(250px, 1fr))` }}>
                        ...
                        {allSpecs.length > 0 && (
                            <div className="col-span-full bg-gray-100 dark:bg-gray-800 p-3 font-bold text-gray-900 dark:text-white text-sm uppercase tracking-wider flex items-center gap-2 mt-4">
                                <span className="w-2 h-6 bg-primary rounded-full"></span> Especificações Técnicas
                            </div>
                        )}
                        ...
                        {allSpecs.map(specKey => (
                            <React.Fragment key={specKey}>
                                <div className="p-4 font-bold text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-800 capitalize bg-gray-50/30 dark:bg-gray-800/30 text-xs md:text-sm">
                                    {specKey.replace(/([A-Z])/g, ' $1').trim()}
                                </div>
                                {products.map(product => (
                                    <div key={product.id} className="p-4 text-sm text-gray-700 dark:text-gray-200 border-b border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-800/30 flex items-center font-mono">
