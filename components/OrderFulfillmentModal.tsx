    ...
    const handleConfirmFulfillment = async () => {
        ...
        try {
            await runTransaction(modularDb, async (transaction) => {
                ...
                const userRef = order.userId ? doc(modularDb, 'users', order.userId) : null;
                ...
                const batchDocs: Record<string, DocumentSnapshot> = {};
                
                for (const batchId of batchIds) {
                    const ref = doc(modularDb, 'products_inventory', batchId);
                    const docSnap = await transaction.get(ref);
                    if (!docSnap.exists()) throw new Error(`Lote ${batchId} não encontrado no sistema.`);
                ...
                if (hasPackages && updatedPackages) {
                    ...
                    // Se não houver pacotes, atualizar os itens diretamente
                    updatedItems = updatedItems.map(item => {
                        if (typeof item === 'string') return item;
                        const orderItem = item as OrderItem;
                        const uniqueId = orderItem.selectedVariant ? `${orderItem.productId}-${orderItem.selectedVariant}` : `${orderItem.productId}`;
                        const serialsForThisItem = scannedItems
                                .filter(s => s.orderItemId === uniqueId)
                                .map(s => s.serialNumber);
                        
                        return {
                            ...orderItem,
                            serialNumbers: serialsForThisItem
                        };
                    });
                }
            ...
            try {
                await fetch('/api/send-push', {
                    ...
                    body: JSON.stringify({
                        title: 'Encomendada Atualizada! 📦',
                        body: isPickup ? 'A sua encomenda já está disponível para levantamento.' : 'A sua encomenda foi enviada!',
                        target: 'specific',
                        specificUserId: order.userId,
                        link: `https://www.all-shop.net/#order/${order.id}`
                    })
                });
