...
const BarcodeScanner: React.FC<BarcodeScannerProps> = ({ onCodeSubmit, onClose, mode }) => {
    ...
    // We don't use videoRef directly for scanning anymore, but we keep it for AI fallback screenshot
    ...
    useEffect(() => {
        const startScanner = async () => {
            ...
            const config = {
                fps: 10,
                qrbox: { width: 250, height: mode === 'serial' ? 100 : 150 },
            ...
            try {
                await html5QrcodeRef.current.start(
                    ...
                    (decodedText) => {
                        ...
                        if (html5QrcodeRef.current) {
                           html5QrcodeRef.current.pause(true); // Pause scanning
                        }
                        onCodeSubmit(decodedText.trim().toUpperCase());
                    },
                    (errorMessage) => {
                        // Regular scanning errors happen constantly as it searches, ignore them
                    }
                );
                
                // Try to get the active track for Zoom and Torch
                setTimeout(() => {
                ...
                setError("Câmara indisponível. Verifique as permissões de acesso.");
            }
        };
    ...
    const toggleTorch = async () => { 
        if (trackRef.current) { 
            ...
            } catch(e) { console.warn("Luz (flash) não suportada neste dispositivo", e); } 
        } 
    };
    ...
    const handleAiScan = async () => {
        ...
        try { 
            ...
            const maxW = 800; // Resize to ensure fast processing
            ...
            else if (msg.includes("API key is missing")) { setError("API_KEY_MISSING"); } 
    ...
    const renderErrorContent = () => { 
        if (error === 'API_KEY_INVALID') return (<div className="flex flex-col items-center w-full"><KeyIcon size={48} className="text-red-500 mb-4" /><h3 className="text-lg font-bold mb-2">Chave API Inválida</h3></div>); 
        else if (error === 'API_KEY_RESTRICTED') return (<div className="flex flex-col items-center w-full"><WifiOff size={48} className="text-yellow-500 mb-4" /><h3 className="text-lg font-bold mb-2">Acesso Bloqueado</h3></div>); 
        return (<><AlertCircle size={40} className="text-red-500 mb-4" /><p className="text-sm font-bold mb-6">{error}</p></>); 
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-sm flex flex-col items-center justify-center p-4">
            ...
            <div className="w-full max-w-sm relative">
                ...
                <div className="relative aspect-[4/3] bg-gray-900 rounded-2xl overflow-hidden border-2 border-white/10 shadow-2xl relative">
                    {/* The HTML5 QR code reader element */}
                    <div id="reader" className="w-full h-full [&>video]:object-cover" style={{minHeight: '100%', width: '100%'}}></div>
                    
                    <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                        {isAiProcessing && <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm rounded-xl">
