...
const OrderDetailsModal: React.FC<OrderDetailsModalProps> = ({ order, onClose, onUpdateOrder, onUpdateTracking, onCopy, inventoryProducts, isAdmin = false }) => {
    ...
    const handleCancelItem = async (productId: number, quantity: number) => {
        console.log("Tentando cancelar item:", { orderId: order.id, productId, quantity });
        if (!window.confirm("Tem a certeza que quer cancelar este item da encomenda? Esta ação reverte o stock.")) return;
        ...
        const result = await cancelOrderItem(order.id, productId, serials, "Cancelamento administrativo");
    ...
    const handleManualPointsAward = async () => { 
        ...
        try { 
            ...
            if (order.userId) { 
                const potentialUserRef = doc(modularDb, 'users', order.userId); 
                const userDoc = await getDoc(potentialUserRef); 
                if (userDoc.exists()) { userRef = potentialUserRef; } 
            } 
            if (!userRef && order.shippingInfo.email) { 
                const q = query(collection(modularDb, 'users'), where('email', '==', (order.shippingInfo.email || '').trim().toLowerCase()), limit(1));
                const userQuerySnapshot = await getDocs(q); 
                if (!userQuerySnapshot.empty) { userRef = userQuerySnapshot.docs[0].ref; } 
            } 
                                ...
                                {isAdmin && isObject && (
                                    <div className="flex items-center gap-2">
                                        <input 
                                            ...
                                            max={itemQty} 
                                            defaultValue={1}
                                            className="w-16 p-1 border rounded text-sm dark:bg-slate-700 dark:text-white"
                                            id={`cancel-qty-${idx}`}
                                        />
                                        <button onClick={() => {
                                            const qtyInput = document.getElementById(`cancel-qty-${idx}`) as HTMLInputElement;
                                            handleCancelItem((item as OrderItem).productId, parseInt(qtyInput.value));
