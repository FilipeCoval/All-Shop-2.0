...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onLogin }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
        ...
        try {
            const docRef = doc(modularDb, "users", firebaseUser.uid);
        ...
        // MELHORIA: Tratar erros de utilizador (senha errada) como avisos, não erros de sistema
        const isUserError = ['auth/invalid-credential', 'auth/user-not-found', 'auth/wrong-password'].includes(err.code);
        
        if (isUserError) {
            console.warn("Tentativa de login falhou (Credenciais):", err.code);
        } else {
            console.error("Erro crítico de autenticação:", err);
        }

        if (err.message && err.message.includes('requests-from-referer')) {
            setError(`Acesso bloqueado pela Google. A sua Chave de API do FIREBASE tem restrições. Vá à Google Cloud > APIs & Serviços > Credenciais, encontre a chave "Browser key" e adicione "${window.location.hostname}" aos "Referenciadores HTTP".`);
        } else if (isUserError) {
