...
import { doc, updateDoc, writeBatch, runTransaction, arrayUnion, getDocs, addDoc, collection, query, where, deleteDoc } from 'firebase/firestore';
...
const ClientDetailsModal: React.FC<ClientDetailsModalProps> = ({ user, orders, onClose, onUpdateUser }) => {
    ...
    const handleSearchDuplicate = async () => {
        ...
        try {
            const snap = await getDocs(query(collection(modularDb, 'users'), where('email', '==', mergeSearchEmail)));
            if (snap.empty) {
    ...
    const handleConfirmMerge = async () => {
        ...
        try {
            ...
            const dupRef = doc(modularDb, 'users', foundDuplicate.uid);
            batch.delete(dupRef);
    ...
    const handleRedeemReward = async (reward: any) => {
        ...
        try {
            ...
            onUpdateUser(user.uid, { loyaltyPoints: updatedPoints, pointsHistory: updatedHistory });
